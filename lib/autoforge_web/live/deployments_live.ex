defmodule AutoforgeWeb.DeploymentsLive do
  ...
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Autoforge.PubSub, "deployment:updated")
    end

    {:ok, assign(socket, page_title: "Deployments", query: "", sort: nil)}
  end
  ...
  def handle_event("search", %{"q" => query}, socket) do
    ...
    {:noreply, push_patch(socket, to: ~p"/deployments?#{params}")}
  end
  ...
  def handle_event("redeploy", %{"id" => id}, socket) do
    deployment = find_deployment(socket, id)
  ...
  def handle_event("destroy", %{"id" => id}, socket) do
    ...
    if deployment do
      Task.Supervisor.start_child(Autoforge.TaskSupervisor, fn ->
        Autoforge.Deployments.DeployOrchestrator.destroy(deployment)
      end)
    end
  ...
  defp load_page(socket, params) do
    ...
    page =
      ...
      |> Ash.Query.load([:project, :vm_instance])
  ...
  defp find_deployment(socket, id) do
    ...
    <Layouts.app flash={@flash} current_user={@current_user} active_page={:deployments}>
      <div>
        <div class="flex items-center justify-between mb-6">
          ...
          <.link navigate={~p"/deployments/new"}>
            <.button variant="solid" color="primary">
              <.icon name="hero-plus" class="w-4 h-4 mr-1" /> New Deployment
            </.button>
          </.link>
        </div>

        <div class="mb-4">
          <.search_bar query={@query} placeholder="Search deployments..." />
        </div>

        <%= if @page.results == [] do %>
          <div class="card bg-base-200">
            <div class="card-body items-center text-center py-12">
              <.icon name="hero-rocket-launch" class="w-10 h-10 text-base-content/30 mb-2" />
              <p class="text-lg font-medium text-base-content/70">No deployments yet</p>
              <p class="text-sm text-base-content/50 mt-1">
                Deploy your first project to a VM to get started.
              </p>
              <.link navigate={~p"/deployments/new"} class="mt-4">
                <.button variant="solid" color="primary" size="sm">
                  <.icon name="hero-plus" class="w-4 h-4 mr-1" /> Create Deployment
                </.button>
              </.link>
            </div>
          </div>
        <% else %>
          <.table>
            ...
            <.table_body>
              <.table_row :for={d <- @page.results}>
                <:cell>
                  <.link navigate={~p"/deployments/#{d.id}"} class="font-medium hover:underline">
                    {d.project && d.project.name}
                  </.link>
                </:cell>
                <:cell>
                  <span class="text-sm text-base-content/70">
                    {d.vm_instance && d.vm_instance.name}
                  </span>
                </:cell>
                <:cell>
                  <span class={"badge badge-sm #{state_badge_class(d.state)}"}>
                    {d.state}
                  </span>
                </:cell>
                <:cell>
                ...
                <:cell class="text-base-content/70 text-sm">
                  <.local_time value={d.inserted_at} user={@current_user} />
                </:cell>
                <:cell>
                  <.dropdown placement="bottom-end">
                    ...
                    <.dropdown_link navigate={~p"/deployments/#{d.id}"}>
                      <.icon name="hero-eye" class="w-4 h-4 mr-2" /> View
                    </.dropdown_link>
                    <.dropdown_button
                      :if={d.state in [:running, :stopped, :error]}
                      phx-click="redeploy"
                      phx-value-id={d.id}
                    >
                      <.icon name="hero-arrow-path" class="w-4 h-4 mr-2" /> Redeploy
                    </.dropdown_button>
                    <.dropdown_separator />
                    <.dropdown_button
                      :if={d.state in [:running, :stopped, :error]}
