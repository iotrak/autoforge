defmodule AutoforgeWeb.VmInstancesLive do
  ...
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Autoforge.PubSub, "vm_instance:updated")
    end

    {:ok, assign(socket, page_title: "VM Instances", query: "", sort: nil)}
  end
  ...
  def handle_event("search", %{"q" => query}, socket) do
    ...
    {:noreply, push_patch(socket, to: ~p"/vm-instances?#{params}")}
  end
  ...
  def handle_event("stop", %{"id" => id}, socket) do
    vm_instance = find_instance(socket, id)

    if vm_instance do
      Task.Supervisor.start_child(Autoforge.TaskSupervisor, fn ->
        Autoforge.Deployments.VmProvisioner.stop(vm_instance)
      end)
    end
  ...
  def handle_event("start", %{"id" => id}, socket) do
    ...
    if vm_instance do
      Task.Supervisor.start_child(Autoforge.TaskSupervisor, fn ->
        Autoforge.Deployments.VmProvisioner.start(vm_instance)
      end)
    end
  ...
  def handle_event("destroy", %{"id" => id}, socket) do
    ...
    if vm_instance do
      Task.Supervisor.start_child(Autoforge.TaskSupervisor, fn ->
        Autoforge.Deployments.VmProvisioner.destroy(vm_instance)
      end)
    end
  ...
  defp find_instance(socket, id) do
    ...
    <Layouts.app flash={@flash} current_user={@current_user} active_page={:vm_instances}>
      <div>
        <div class="flex items-center justify-between mb-6">
          <div>
            <h1 class="text-2xl font-bold tracking-tight">VM Instances</h1>
            <p class="mt-2 text-base-content/70">
              Manage your GCE virtual machines.
            </p>
          </div>
          <.link navigate={~p"/vm-instances/new"}>
            <.button variant="solid" color="primary">
              <.icon name="hero-plus" class="w-4 h-4 mr-1" /> New VM
            </.button>
          </.link>
        </div>

        <div class="mb-4">
          <.search_bar query={@query} placeholder="Search VM instances..." />
        </div>

        <%= if @page.results == [] do %>
          <div class="card bg-base-200">
            <div class="card-body items-center text-center py-12">
              <.icon name="hero-server" class="w-10 h-10 text-base-content/30 mb-2" />
              <p class="text-lg font-medium text-base-content/70">No VM instances yet</p>
              <p class="text-sm text-base-content/50 mt-1">
                Create your first VM instance to get started.
              </p>
              <.link navigate={~p"/vm-instances/new"} class="mt-4">
                <.button variant="solid" color="primary" size="sm">
                  <.icon name="hero-plus" class="w-4 h-4 mr-1" /> Create VM
                </.button>
              </.link>
            </div>
          </div>
        <% else %>
          <.table>
            ...
            <.table_body>
              <.table_row :for={vm <- @page.results}>
                <:cell>
                  <.link navigate={~p"/vm-instances/#{vm.id}"} class="font-medium hover:underline">
                    {vm.name}
                  </.link>
                </:cell>
                <:cell>
                  <span class="text-sm text-base-content/70">
                    {vm.vm_template && vm.vm_template.name}
                  </span>
                </:cell>
                <:cell>
                  <span class={"badge badge-sm #{state_badge_class(vm.state)}"}>
                    {vm.state}
                  </span>
                </:cell>
                <:cell>
                ...
                <:cell class="text-base-content/70 text-sm">
                  <.local_time value={vm.inserted_at} user={@current_user} />
                </:cell>
                <:cell>
                  <.dropdown placement="bottom-end">
                    ...
                    <.dropdown_link navigate={~p"/vm-instances/#{vm.id}"}>
                    ...
                    <.dropdown_separator />
                    <.dropdown_button
                      :if={vm.state in [:running, :stopped, :error]}
                      phx-click="destroy"
                      phx-value-id={vm.id}
