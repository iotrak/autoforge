defmodule AutoforgeWeb.VmTemplatesLive do
  ...
  def mount(_params, _session, socket) do
    {:ok, assign(socket, page_title: "VM Templates", query: "", sort: nil)}
  end
  ...
  def handle_event("search", %{"q" => query}, socket) do
    ...
    {:noreply, push_patch(socket, to: ~p"/vm-templates?#{params}")}
  end
    ...
    <Layouts.app flash={@flash} current_user={@current_user} active_page={:vm_templates}>
      <div>
        <div class="flex items-center justify-between mb-6">
          <div>
            <h1 class="text-2xl font-bold tracking-tight">VM Templates</h1>
            <p class="mt-2 text-base-content/70">
              Manage reusable GCE VM configurations for deployments.
            </p>
          </div>
          <.link navigate={~p"/vm-templates/new"}>
        ...
        <div class="mb-4">
          <.search_bar query={@query} placeholder="Search VM templates..." />
        </div>

        <%= if @page.results == [] do %>
          <div class="card bg-base-200">
            <div class="card-body items-center text-center py-12">
              <.icon name="hero-square-3-stack-3d" class="w-10 h-10 text-base-content/30 mb-2" />
              <p class="text-lg font-medium text-base-content/70">No VM templates yet</p>
              <p class="text-sm text-base-content/50 mt-1">
                Create your first VM template to get started.
              </p>
              <.link navigate={~p"/vm-templates/new"} class="mt-4">
        ...
        <% else %>
          <.table>
            <.table_head>
              ...
              <:col>
                <.sort_header column="machine_type" sort={@sort}>Machine Type</.sort_header>
              </:col>
              <:col>Zone</:col>
              <:col>Disk</:col>
              <:col>
            ...
            <.table_body>
              <.table_row :for={template <- @page.results}>
                ...
                <:cell>
                  <span class="text-sm font-mono">{template.machine_type}</span>
                </:cell>
                <:cell>
                  <span class="text-sm text-base-content/70">{template.zone}</span>
                </:cell>
                <:cell>
                  <span class="text-sm text-base-content/70">
                    {template.disk_size_gb} GB {template.disk_type}
                  </span>
                </:cell>
                ...
                <:cell>
                  <.dropdown placement="bottom-end">
                    ...
                    <.dropdown_link navigate={~p"/vm-templates/#{template.id}/edit"}>
                    ...
                    <.dropdown_button
                      ...
                      data-confirm="Are you sure you want to delete this VM template?"
                      class="text-error"
                    >
